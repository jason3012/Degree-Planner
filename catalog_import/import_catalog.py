...
def parse_core_tags(core_column: str) -> list[str]:
    """Parse core tags from CSV column: split by comma, trim, skip empty/None."""
        ...
        with transaction.atomic():
            for row in reader:
                try:
                    ...
                    # Core tags from CSV (authoritative eligible-by-core)
                    core_col = row.get('satisfies_core_requirement', '') or row.get('Satisfies Core Requirement', '')
                    tag_names = parse_core_tags(core_col)
                    tag_objs = [CoreTag.objects.get_or_create(name=name)[0] for name in tag_names]
